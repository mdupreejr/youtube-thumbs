...
class YouTubeAPI:
    ...
    def _calculate_title_similarity(self, result_title: str, query_title: str) -> float:
        """
    ...
    def _score_and_sort_results(self, items: list, title: str) -> list:
        """
        ...
        for item in items:
            ...
            score = self._calculate_title_similarity(result_title, title)
    ...
    def _log_search_api_call(self, search_query: str, title: str, success: bool, results_count: int = 0, error_message: str = None):
        """
    ...
    def _log_batch_api_call(self, phase: str, batch_num: int, video_count: int, title: str, success: bool, error_message: str = None):
        """
    ...
    def _fetch_video_batch(
        ...
        try:
            ...
            self._log_batch_api_call(phase, batch_num, videos_fetched, title, success=True)
            ...
            self._log_batch_api_call(phase, batch_num, quota_cost, title, success=False, error_message=error_msg)
    ...
    def search_video_globally(self, title: str, expected_duration: Optional[int] = None, artist: Optional[str] = None, return_api_response: bool = False):
        ...
        try:
            ...
            # Track API usage
            self._log_search_api_call(search_query, title, success=True, results_count=len(response.get('items', [])))
            ...
            video_ids = self._score_and_sort_results(items, title)
            ...
            if phase1_ids:
                batch_candidates, batch_all_videos, batch_count = self._fetch_video_batch(
                    phase1_ids, expected_duration, title, "Phase 1", 1, api_debug_data
                )
            ...
            if not candidates and len(video_ids) > PHASE_1_LIMIT:
                ...
                if phase2_ids:
                    batch_candidates, batch_all_videos, batch_count = self._fetch_video_batch(
                        phase2_ids, expected_duration, title, "Phase 2", 2, api_debug_data
                    )
                    candidates.extend(batch_candidates)
                    all_fetched_videos.extend(batch_all_videos)
                    videos_checked += batch_count
